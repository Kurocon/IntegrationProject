...
-- Switch according to datatype.
   ...
   elseif datatype == 1 then
      ...
      pinfo.cols.info = sVersion .. "Broadcast Message from " .. buffer(24,nickLen):string() .. "@" .. buffer(279,hostLen):string() .. " with IP " .. buffer(0,1):uint() .. "." .. buffer(1,1):uint() .. "." .. buffer(2,1):uint() .. "." .. buffer(3,1):uint()
   ...
   elseif datatype == 3 then
      local pChatLen = buffer(20,2):uint()-4
      pinfo.cols.info = sVersion .. "Private Chat Message from " .. buffer(0,1):uint() .. "." .. buffer(1,1):uint() .. "." .. buffer(2,1):uint() .. "." .. buffer(3,1):uint() .. " to " .. buffer(24,1):uint() .. "." .. buffer(25,1):uint() .. "." .. buffer(26,1):uint() .. "." .. buffer(27,1):uint() .. ": " .. buffer(24,pChatLen):string()
